...
async function startVenom() {
  try {
    // Busca caminho do Chromium dinâmico do Puppeteer
    const browserPath = await puppeteer.executablePath();

    venom
      .create({
        session: "apizap",
        headless: "new", // compatibilidade com Chrome recente
        useChrome: false, // usar Chromium Puppeteer
        browserPath: browserPath,
        chromiumArgs: ["--no-sandbox", "--disable-setuid-sandbox"], // importante no Render
        logQR: false,
    ...
    console.error("Erro ao obter caminho do Chromium:", err);
  }
}
...
const start = (client) => {
  app.post("/send-message", async (req, res) => {
    ...
    if (!to || !message) {
      return res
        .status(400)
        .json({ error: "Campos 'to' e 'message' obrigatórios." });
    }
